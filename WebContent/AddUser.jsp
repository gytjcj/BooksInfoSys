...
body {margin:0; padding:0; background:#EEB2EE;}

.form-bak {width:800px; height:500px; margin:40px auto; background:url('jpg/10.jpg'); border-radius:10px; box-shadow:0 2px 8px #999;}
...
.input {float:right; width:278px; height:45px; background:#f6f6f6; border:1px solid #999; border-radius:10px; box-shadow:inset 0 2px 5px #999; margin:10px 0; position:relative; }
...
font-size:40px; height:200px; width:200px; background:#00FF7F; margin:20px 30px 0 30px; color:#eee;}
...
#top h1 {font-size:30px; line-height:40px; padding:10px; text-align:center; background:url('jpg/10.jpg');}
...
if(!RegEmail.test(account)) {lineState("account","error","号码格式不正确"); flags[0]=false;enableSubmit(false);}
...
function v_accountr(){
	...
	var accountr = $("#accountr").val();

	if(accountr.length==0) {lineState("accountr","error","格式不正确"); flags[4]=false;}

	else{

	if(accountr.length>1){lineState("accountr","error","格式不正确"); flags[4]=false;}

	else{

	lineState("accountr","corect","");
...
<body style="background-image: url('jpg/add.gif')">
<div id="top"><h1>用户注册</h1></div>

<div class="form-bak">

<form method="post" action="/BooksInfoSys/Users?type=AddUser" onsubmit="adaptValue();">
...
<div class="line" id="line_accountr">

<div class="info">

<strong>性别</strong>

<span class="tips">请输入您的性别</span>

</div>

<div class="input">

<input type="text" name="accountr" id="accountr" onblur="v_accountr();" onkeyup="v_accountr();"/>
...
<input type="submit" id="submit" value="完成注册" disabled="disabled"  onclick="alert('注册成功')"/>
...
<div>已经拥有账号，<a href="/BooksInfoSys/UserLogin.jsp">直接登录</a></div>
